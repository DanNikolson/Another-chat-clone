...
class Chat extends Command
{
    ...
    public function handle()
    {
        ...
        $chatBoxSystemInstruction = 'You are anotherChatClone. Answer as concisely as possible.';
        ...
        intro('Welcome to anotherChatClone!');

        // Ask the user to choose a GPT model
        $chosenModel = suggest(
            label: 'Choose a GPT model: ',
            options: array_values($availableGPTModels),
            required: 'This field is required. Choose a model to use for anotherChatClone.',
            hint: 'Choose a model to use for anotherChatClone.'
        );
        ...
        $chosenMaxTokens = text(
            ...
            required: 'This field is required. Choose max tokens for anotherChatClone.',
            default: 600,
            hint: 'Choose max tokens for anotherChatClone.'
        );

        // Ask the user to choose the temperature
        $chosenTemperature = text(
            label: 'Choose temperature: ',
            required: 'This field is required. Choose temperature for anotherChatClone.',
            default: 0.6,
            hint: 'Choose temperature for anotherChatClone.'
        );

        // Get the key of the chosen model
        $chosenModelKey = array_search($chosenModel, $availableGPTModels);

        // Ask the user to ask a question to anotherChatClone
        $question = text(
            label: 'Ask anotherChatClone: ',
            required: 'This field is required. Type "exit" to leave the chat.',
            hint: 'Ask whatever you want anotherChatClone or type "exit" to exit the chat.'
        );
        ...
        outro('Thank you for using anotherChatClone. See you next time!');
    }
}
